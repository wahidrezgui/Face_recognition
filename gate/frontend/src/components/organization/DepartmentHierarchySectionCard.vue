...
export default {
  ...
  computed: {
    rootLabel() {
      return nodeLabel(this.rootNode);
    },
    searchPlaceholder() {
      return `ابحث في ${this.rootLabel}...`;
    },
    section() {
      const filtered = filterDepartmentTree([this.rootNode], this.treeFilter);
      const sections = buildHierarchySections(filtered);
  ...
  methods: {
    ...
    applyDefaultCollapse() {
      const sections = buildHierarchySections([this.rootNode]);
      const roots = collectChartRootsFromSections(sections);
