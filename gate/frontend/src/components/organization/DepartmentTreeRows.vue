<template>
  <ul class="dept-tree-rows" :class="{ 'dept-tree-rows--root': depth === 0 }">
    <li v-for="node in nodes" :key="node.key ?? node.id" class="dept-tree-rows__item">
      <div class="dept-tree-row" :class="rowClasses(node)">
        <button
          ...
          @click="toggle(node)"
        >
          <i :class="isExpanded(node) ? 'pi pi-chevron-up' : 'pi pi-chevron-down'" aria-hidden="true" />
        </button>
        <span v-else class="dept-tree-row__toggle dept-tree-row__toggle--spacer" aria-hidden="true" />
        ...
        <div class="dept-tree-row__titles">
          ...
          <span
            v-if="node.name_en && node.name_en !== nodeLabel(node)"
            class="dept-tree-row__subtitle"
            dir="ltr"
          >
            {{ node.name_en }}
          </span>
          <div v-if="linkedBases(node).length" class="dept-tree-row__bases">
            <span
        ...
        <span v-if="node.childCount" class="dept-tree-row__count" :title="`${node.childCount} فرع`">
          {{ node.childCount }}
        </span>
...
<script>
import { baseLabel, linkedBases, nodeLabel, nodeId } from '../../lib/organization/departmentTreeHelpers';
...
.dept-tree-row--depth-0 { border-inline-start-color: #8b1538; }
.dept-tree-row--depth-1 { border-inline-start-color: #2563eb; }
.dept-tree-row--depth-2 { border-inline-start-color: #059669; }
.dept-tree-row--depth-3 { border-inline-start-color: #d97706; }
.dept-tree-row--depth-4 { border-inline-start-color: #64748b; }
...
.dept-tree-row__action--add { color: #8b1538; }
.dept-tree-row__action--add:hover { background: rgba(139, 21, 56, 0.08); }
.dept-tree-row__action--edit { color: #2563eb; }
.dept-tree-row__action--edit:hover { background: #eff6ff; }
.dept-tree-row__action--delete { color: #dc2626; }
.dept-tree-row__action--delete:hover { background: #fef2f2; }
</style>
