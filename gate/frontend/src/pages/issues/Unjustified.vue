<template>
  <PageContainer title="Unjustified Issues" description="بدون عذr">
...
import Toast from 'primevue/toast';
  ...
  export default {
    mixins: [createReportPageMixin('unjustified')],
    components: {
      ReportNavCards,
      ReportSearchPanel,
      ReportDataGrid,
    ...
    methods: {
      ...
      onBtPdf() {
        ...
        .then(response => {
          ...
          printWindow.document.write('<h1>تقرير بدون عذر</h1>');
            ...
            this.RawData.forEach(row => {
              printWindow.document.write('<tr>');
                ...
                reorderedColumns.forEach(col => {
                  ...
                  // Always write an empty string for the "notes" column
                  const cellValue = col.field === 'notes' ? '' : row[col.field];
      
                  printWindow.document.write(`<td style="${style}">${cellValue}</td>`);
                });
      ...
      onBtExport() {
        ...
        .then(response => {
          ...
          this.$refs.reportGrid.exportDataAsCsv({
            ...
            fileName: 'Unjustified-Report.csv', // Customize file name
          });
        })
        .finally(() => {
          this.isLoading = false; // Stop loading indicator
