<template>
  <PageContainer title="Justified Issues" description="بعذر">
...
import Toast from 'primevue/toast';
  ...
  export default {
    mixins: [createReportPageMixin('justified')],
    components: {
      ReportNavCards,
      ReportSearchPanel,
      ReportDataGrid,
    ...
    methods: {
      ...
      formatDateString(dateString) {
          ...
          if (parts.length < 3) {
              //console.error("Date string does not have the expected format.");
              return null; // Handle the case where the format is incorrect
          }

          const day = parts[0]; // Get the day
          const month = months[parts[1]]; // Get the month abbreviation

          // Check if month is undefined
          if (!month) {
              //console.error("Invalid month:", parts[1]); // Log an error if the month is invalid
      ...
      deleteNote() {
          if (confirm("هل أنت متأكد أنك تريد حذف هذه الملاحظة؟")) {
              // Log the guest ID and date of the note
              //console.log("Guest ID to delete note:", this.note.emp_id);
              
              // Format the date using the utility function
              const formattedDate = this.formatDateString(this.note.day);
              //console.log("Formatted date of the note to delete:", formattedDate);

              // Make the API call to delete the note
              api.delete(`/api/employees/notes`, {
                  data: {
                      emp_id: this.note.emp_id,
                      day: formattedDate, // Use the formatted date here
                      created_by:localStorage.getItem('user_name')
                  }
              })
              .then(response => {
                  //console.log(response.data.message); // Log the success message
                  this.guest.notes = ''; // Clear the notes display if needed
                  this.getEmployees(); // Refresh the data if needed
              })
              .catch(error => {
      ...
      onBtPdf() {
        ...
        .then(response => {
          ...
          printWindow.document.write('<h1>تقرير بعذر</h1>');
      ...
      onBtExport() {
        ...
        this.searchPreset.fetchFn(this.buildExportParams('EXCEL generated'))
        .then(response => {
          ...
          this.$refs.reportGrid.exportDataAsCsv({
            ...
            fileName: 'Justified-Report.csv', // Customize file name
          });
        })
        .finally(() => {
          this.isLoading = false; // Stop loading indicator
