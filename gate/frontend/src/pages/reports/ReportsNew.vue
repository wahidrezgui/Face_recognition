    ...
    <PageContainer title="Reports Dashboard" description="Overview and daily movement reports">
            ...
            <div class="text-green-900 border-green-500 bg-green-200 border-b-2 card w-full cursor-pointer rounded-md border flex">
                ...
                <div class="block p-2 w-full">
                    ...
                    <h2 class="font-normal text-gray-400 text-md mt-1">تقارير التسجيل</h2>
                </div>
            </div>
            ...
            <router-link to="/issues" >
                <div  class="bg-white border-b-2 card w-full cursor-pointer rounded-md border flex" >
                    ...
                    <div class="block p-2 w-full">
                        ...
                        <h2 class="font-normal text-gray-400 text-md mt-1"> تقارير التأخير اليومية </h2>
                    </div>
                </div>
            </router-link>
            <router-link to="/exportReports" ><div  class="bg-white border-b-2 card w-full cursor-pointer rounded-md border flex" >
            <div class="p-2 max-w-sm">
            <div class="bg-neutral-500 text-center rounded-full w-14 h-14 text-lg p-3 text-white mx-auto" >
            ...
            <div class="block p-2 w-full">
            <p class="font-semibold text-gray-900 dark:text-gray-200 text-xl">
            Export Issues Reports
            </p>
            <h5 class="font-normal text-gray-400 text-md mt-1">استخراج تقارير التأخير</h5>
            </div>
            </div></router-link>
            <router-link to="/unjustified">
            <div class="bg-white border-b-2 card w-full cursor-pointer rounded-md border flex">
                <div class="p-2 max-w-sm">
                    <div class="bg-neutral-500 text-center rounded-full w-14 h-14 text-lg p-3 text-white mx-auto">
                        <span><i class="pi pi-flag" style="font-size:20px; color:red;"></i></span>
                    </div>
                </div>
                <div class="block p-2 w-full">
                    <p class="font-semibold text-gray-900 dark:text-gray-200 text-xl">Unjustified Issues</p>
                    ...
                    <div class="text-gray-700">
                        <div class=" text-sm">
                            <div class="grid grid-cols-4">
                                ...
                                <div class="px-4 py-2 col-span-3">{{guest.day}}</div>
                            </div>
                            <div class="grid grid-cols-4">
                                <div class="py-2 font-semibold">First checkin</div>
                                <div class="px-4 py-2 col-span-3">{{guest.checkin}}</div>
                            </div>
                            <div class="grid grid-cols-4">
                                <div class="py-2 font-semibold">Last Checkout</div>
                                <div class="px-4 py-2 col-span-3">{{guest.checkout}}</div>
                            </div>
                            <div class="grid grid-cols-4">
                                <div class="py-2 font-semibold">Status</div>
                                <div class="px-4 py-2 col-span-3" v-html="guest.defaut"></div>
                            </div>
                            <div class="grid grid-cols-4">
                                <div class="py-2 font-semibold">Notes</div>
                                <div v-if="guest.notes!=''" class="px-4 py-2 col-span-3" v-html="guest.notes"></div>
                                <div v-else class="col-span-3">
                                    <form novalidate="" @submit.prevent="addNotes">
                                        <div class="flex items-center">
                                            <textarea v-model="note.notes" id="notes" rows="3" class="block mx-4 p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your message..."></textarea>
...
export default {
        ...
        computed: {
            mergedColumnDefs() {
                    ...
                    const columnIn = modifiedColumnDefs.findIndex( (column) => column.field === "checkin_msg" );
                    if (columnIn !== -1) { modifiedColumnDefs[columnIn].cellRenderer = customCellRendererIn;  }

                    const columnOut = modifiedColumnDefs.findIndex( (column) => column.field === "checkout_msg" );
        ...
        methods: {
                ...
                fetchData(){
                    this.isLoading = true;
                     api.get('/api/reports/attendance?type=day', {
                        params: {
                            dep_user_id: this.depId,
                            day: this.day,
                        },
                    })
                    .then(response => {
