<template>
  <PageContainer title="Export Issues Reports" description="استخراج تقارير التأخير">
...
import Toast from 'primevue/toast';
  ...
  export default {
    mixins: [createReportPageMixin('export')],
    components: {
      ReportNavCards,
      ReportSearchPanel,
      ReportDataGrid,
    ...
    methods: {
      ...
      onBtPdf() {
        ...
        .then(response => {
          ...
          printWindow.document.write('<h1>استخراج تقارير التأخير</h1>');
      ...
      onBtExport() {
        ...
        .then(response => {
          ...
          this.$refs.reportGrid.exportDataAsCsv({
            ...
            fileName: 'ExportReports-Report.csv', // Customize file name
          });
        })
        .finally(() => {
          this.isLoading = false; // Stop loading indicator
