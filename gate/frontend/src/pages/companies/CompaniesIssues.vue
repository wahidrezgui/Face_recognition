<template>
    <PageContainer title="Company Issues" description="Daily issues reports for companies">

            <div class="wrapper-card grid lg:grid-cols-6 grid-cols-1 md:grid-cols-2 gap-2 mb-5"  >
            
            <router-link to="/companies-reporting" ><div  class="bg-white border-b-2 card w-full cursor-pointer rounded-md border flex" >
            ...
            <div class="flex items-start justify-between">
                ...
                <div class="grid grid-cols-12 gap-x-4 ">
                    <div class="col-span-3">
                        <TreeSelect v-model="issues.dep_id" :options="departments" @change="fetchData" placeholder="Select Company" showClear class="w-full md:w-14rem border border-dark-200" />
                    </div>
                    <div class="col-span-3">
                       <input type="date" v-model="issues.day" @change="fetchData" placeholder="Filter by date" class="w-full h-12 rounded border border-gray-200"  />
                    </div>
                    <div class="col-span-3">
                        <div class="card flex flex-wrap justify-content-center gap-3">
                        <div class="flex align-items-center mt-2">
                            <Checkbox v-model="issues.mvtype" inputId="checkin" value="Check-In" @change="fetchData" />
                            <label for="checkin" class="ml-2"> Check-In </label>
                        </div>
                        <div class="flex align-items-center mt-2">
                            <Checkbox v-model="issues.mvtype" inputId="checkout" value="Check-Out" @change="fetchData" />
                ...
                <div class="mt-5 mb-5 sm:ml-6 sm:mt-0 sm:flex sm:shrink-0 sm:items-center" >
                <!-- <button v-if="RawData.length>0" @click="printIssues" type="button" class="mx-5 inline-flex text-sm bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded">
            ...
            <div id="printIssues">
                ...
                <div>
                <Paginator ref="myPaginator" :rows="perPage" :totalRecords="totalRows" @click="onPageChange"></Paginator>
                </div>
...
function customCellRendererIn(params) {
    var cellValue = params.value;
     ...
     {return '<span class="font-bold text-red p-4">'+params.value.substring(1)+'</span>';} 
     else
...
export default {
        components: {
                    AgGridVue, Toast, Dropdown, Checkbox, Card, Avatar, TreeSelect, Paginator, PageContainer
                    },
        data() {
            return {
                activeTab:10,
        ...
        mounted() {
            const d = new Date();
            this.issues.day = d.getFullYear() + '-' + (d.getMonth()+1) + '-' + d.getDate();
           // this.today = d.getFullYear() + '-' + (d.getMonth()+1) + '-' + d.getDate();

           const now = new Date();
            const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true };
            this.today = now.toLocaleString('ar-QA', options);
        ...
        methods: {
                printIssues(){
                    ...
                    const pdfConfig = {
                                        ...
                                        containerDiv.style.padding = '10mm';
                                        containerDiv.style.backgroundColor = 'white';

                                        containerDiv.setAttribute('dir', 'rtl');

                                        containerDiv.innerHTML = `<center><img src="/armed.png" style="height:100px;" ></center>`;
                                        containerDiv.innerHTML += `<center><h2 style="font-size:30px;"> تقرير الدخول و الخروج </h2></center>`;
                                        containerDiv.innerHTML += `<center><h2>تاريخ الطباعة  : ${this.today} </h2></center><br><br>`;

                                        this.RawData.forEach(user => {
                                            const userDiv = document.createElement('div');

                                            const checkinColorStyle = user.checkin.charAt(0) === '.' ? 'color: red;' : '';
                                            const checkoutColorStyle = user.checkout.charAt(0) === '.' ? 'color: red;' : '';

                                            userDiv.innerHTML = `<table style="font-size:14px; width:100%;"><tbody><tr><td style="padding-left:50px;"><b>Full Name</b></td><td>${user.fullname_en}</td><td><b>Company</b></td><td>${user.department}</td><td><b>اليوم</b></td><td>${this.issues.day}</td></tr></tbody></table>`;
                                            userDiv.innerHTML += `<br>`;
                                                const checkinData = user.checkin.split('**');
                                                if(user.checkin!=''){
                                                userDiv.innerHTML += `<table style="font-size:14px; width:100%; margin-bottom:10px;"><thead><tr><td style="width:100px;"></td><td style="width:150px;">التوقيت</td><td style="width:150px;">التسجيل</td><td style="width:200px;">البوابة</td><td style="width:200px;">القاعدة</td></tr></thead><tbody style="${checkinColorStyle}"><tr><td>الدخول</td><td>${checkinData[0]}</td><td>${checkinData[1]}</td><td>${checkinData[2]}</td><td>${checkinData[3]}</td></tr></tbody></table>`;    
                                                }
                                                
                                                const checkoutData = user.checkout.split('**');
                                                if(user.checkout!=''){
                                                  userDiv.innerHTML += `<table style="font-size:14px; width:100%;"><thead><tr><td style="width:100px;"></td><td style="width:150px;">التوقيت</td><td style="width:150px;">التسجيل</td><td style="width:200px;">البوابة</td><td style="width:200px;">القاعدة</td></tr></thead><tbody style="${checkoutColorStyle}"><tr><td>الخروج</td><td>${checkoutData[0]}</td><td>${checkoutData[1]}</td><td>${checkoutData[2]}</td><td>${checkoutData[3]}</td></tr></tbody></table>`;   
                                                }
                ...
                fetchData(){
                    ...
                    api.get('/api/companies/'+this.depId) 
                    .then(response => {
                    ...
                    api.get('/api/dashboard-reports/companies/issues',{
                        params: {
                            dep_parent_id:this.depId,
                            dep_id: Object.keys(this.issues.dep_id)[0],
                            day: this.issues.day,
                            mvtype: this.issues.mvtype,
                ...
                OnClicked(event){
                        ...
                        api.get('/api/employees/'+id+'?day='+this.issues.day) 
                        .then(response => {
