...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach (['employees', 'logs', 'employee_notes'] as $tableName) {
            if (Schema::hasColumn($tableName, 'created_by_legacy')) {
        ...
        DB::statement($this->notesViewSql());
    }
    ...
    public function down(): void
    {
        ...
        foreach (['employees', 'logs', 'employee_notes'] as $tableName) {
            if (! Schema::hasColumn($tableName, 'created_by_legacy')) {
                Schema::table($tableName, function (Blueprint $table) {
                    $table->string('created_by_legacy')->nullable();
                });
            }
        }

        DB::statement($this->legacyNotesViewSql());
    }

    private function notesViewSql(): string
    {
        return <<<'SQL'
...
SELECT
    ...
    COALESCE(
        NULLIF(TRIM(CONCAT(COALESCE(note_users.firstname, ''), ' ', COALESCE(note_users.lastname, ''))), ''),
        note_users.username
    ) AS created_by,
...
LEFT JOIN users AS note_users ON notes.created_by_id = note_users.id
...
SQL;
    ...
    private function legacyNotesViewSql(): string
    {
        return <<<'SQL'
CREATE VIEW employee_specific_movements_notes_view AS
WITH FirstCheckIn AS (
