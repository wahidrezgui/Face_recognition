...
/// <summary>
/// Fan-out pub/sub for SSE: separate live (desk/dashboard) and slim (gymapp) subscriber pools.
/// </summary>
public class GateChannelRegistry
{
    private readonly StreamHub _live = new();
    private readonly StreamHub _slim = new();

    public (Guid SubscriptionId, ChannelReader<GateEvent> Reader) SubscribeGateLive(string gateId) =>
        _live.SubscribeGate(gateId);

    public (Guid SubscriptionId, ChannelReader<GateEvent> Reader) SubscribeAllLive() =>
        _live.SubscribeAll();

    public void UnsubscribeGateLive(string gateId, Guid subscriptionId) =>
        _live.UnsubscribeGate(gateId, subscriptionId);

    public void UnsubscribeAllLive(Guid subscriptionId) =>
        _live.UnsubscribeAll(subscriptionId);

    public (Guid SubscriptionId, ChannelReader<GateEvent> Reader) SubscribeGateSlim(string gateId) =>
        _slim.SubscribeGate(gateId);

    public (Guid SubscriptionId, ChannelReader<GateEvent> Reader) SubscribeAllSlim() =>
        _slim.SubscribeAll();

    public void UnsubscribeGateSlim(string gateId, Guid subscriptionId) =>
        _slim.UnsubscribeGate(gateId, subscriptionId);

    public void UnsubscribeAllSlim(Guid subscriptionId) =>
        _slim.UnsubscribeAll(subscriptionId);

    public void PublishLive(string gateId, GateEvent evt) => _live.Publish(gateId, evt);

    public void PublishSlim(string gateId, GateEvent evt) => _slim.Publish(gateId, evt);

    public IEnumerable<string> ActiveGateIds => _live.ActiveGateIds;

    // Backward-compatible aliases (live stream)
    public (Guid SubscriptionId, ChannelReader<GateEvent> Reader) SubscribeGate(string gateId) =>
        SubscribeGateLive(gateId);
    ...
    public void UnsubscribeGate(string gateId, Guid subscriptionId) =>
        UnsubscribeGateLive(gateId, subscriptionId);

    public void UnsubscribeAll(Guid subscriptionId) =>
        UnsubscribeAllLive(subscriptionId);

    public void Publish(string gateId, GateEvent evt) =>
        PublishLive(gateId, evt);

    private sealed class StreamHub
    {
