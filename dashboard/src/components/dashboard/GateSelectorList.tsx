  ...
  return (
    <div className="mt-auto">
      ...
      {gates.map((g) => (
        <div
          key={g.id}
          onClick={() => onSelectGate(selectedGate === g.id ? undefined : g.id)}
          className={`flex cursor-pointer items-center gap-2.5 border-b border-[#111f33] px-4 py-2.5 hover:bg-[#0d1a2f] ${selectedGate === g.id ? "bg-[#0d1a2f]" : ""}`}
        >
          <IconDot online={g.online && (g.status?.camera_open ?? false)} />
          <div>
            <p className="text-xs font-medium text-gray-200">{g.name}</p>
            <p className="text-[10px] text-gray-600">{selectedGate === g.id ? "Selected" : "Click to filter"}</p>
          </div>
          <span
