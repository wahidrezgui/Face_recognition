...
if os.path.isfile(_config_path):
    try:
        with open(_config_path) as f:
            _cfg = json.load(f)
        if "camera_source" in _cfg and "GV_CAMERA_SOURCE" not in os.environ:
            settings.camera_source = str(_cfg["camera_source"])
            logger.info("Loaded camera_source from config: %s", settings.camera_source)
        if "direction" in _cfg and "GV_DIRECTION" not in os.environ:
...
if os.path.isfile(_python_settings_path):
    try:
        ...
        if "motion_threshold" in _ps and "GV_MOTION_THRESHOLD" not in os.environ:
        ...
        if "motion_pixel_threshold" in _ps and "GV_MOTION_PIXEL_THRESHOLD" not in os.environ:
